#!/usr/bin/env python3
"""
Calibration v3: Re-run extraction on 50 fresh documents after 6 v2 bug fixes.

Selects 10 random documents per era, excluding all previous calibration samples,
...
# Previous sample files to exclude
PREV_SAMPLE_FILES = [
    "data/calibration_sample_ids.json",
    "data/calibration_v2_sample_ids.json",
]
...
def load_previous_sample_ids():
    """Load document IDs from all previous calibrations to exclude them."""
    all_ids = set()
    for filepath in PREV_SAMPLE_FILES:
        try:
            with open(filepath) as f:
                data = json.load(f)
            all_ids.update(item[0] for item in data)
...
def select_sample(exclude_ids: set[int]) -> list[dict]:
    """Select 10 random documents per era, excluding previous samples."""
    conn = get_connection()
    cursor = conn.cursor()
    ...
    for i, doc_row in enumerate(docs, 1):
        ...
        try:
            ...
            if doc.id and not doc.id.startswith("UNK-"):
                result_entry["self_citation_filtered"] = True
...
def main():
    print("=" * 60)
    print("CALIBRATION v3: Post v2-Fix Extraction Review")
    print("=" * 60)

    # Load previous samples to exclude
    prev_ids = load_previous_sample_ids()
    print(f"\nExcluding {len(prev_ids)} documents from previous calibrations (v1+v2)")
    ...
    with open("data/calibration_v3_sample_ids.json", "w") as f:
        json.dump(sample_ids, f, indent=2)
    print("Sample IDs saved to data/calibration_v3_sample_ids.json")
    ...
    with open("data/calibration_v3_results.json", "w") as f:
        json.dump(output, f, indent=2)
    print(f"\nFull results saved to data/calibration_v3_results.json")
